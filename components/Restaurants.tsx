...
const restaurants: Restaurant[] = [
  ...
  },
  {
    name: 'SushiriaSakura',
    rating: 4.7,
    image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQGH3w_zC3ky1DKpxWeDABC-4IsAK1uJudqNA&s',
    distance: '1.5 km',
  },
  {
    name: 'HamburgueriaSabor',
    rating: 4.3,
    image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSdVLYPA2AALXGb4-LjbXfUwAmtdsKFOC4KBXJ0hJOfvJM4Q6bEpDx858GirSgKUQYAoFg&usqp=CAU',
    distance: '3 km',
  },
  {
    name: 'ChinainBox',
    rating: 4.3,
    image: 'https://upload.wikimedia.org/wikipedia/pt/thumb/7/7d/Logo-china_in_china.jpg/375px-Logo-china_in_china.jpg',
    distance: '3 km',
  },
  ...
  return (
    <View style={styles.restaurants}>
      ...
      {restaurants.map((restaurant, index) => {
        ...
        if (restaurant.name === 'SushiriaSakura') {
          return (
            <Link href="/(tabs)/screens/SushiriaSakura" asChild key={index}>
              <TouchableOpacity>{cardContent}</TouchableOpacity>
            </Link>
          );
        }
        if (restaurant.name === 'HamburgueriaSabor') {
          return (
            <Link href="/(tabs)/screens/HamburgueriaSabor" asChild key={index}>
              <TouchableOpacity>{cardContent}</TouchableOpacity>
            </Link>
          );
        }
        if (restaurant.name === 'ChinainBox') {
          return (
            <Link href="/(tabs)/screens/ChinainBox" asChild key={index}>
              <TouchableOpacity>{cardContent}</TouchableOpacity>
            </Link>
          );
        }
